...
**Book content** is the polished output. It lives in `weeks/`, `controversies/`, `seminar-chinese-critique.qmd`, the appendices, and a few top-level files like `index.qmd` and `where-this-lands.qmd`. It's written in Quarto Markdown (`.qmd`), which is regular Markdown plus YAML front matter and executable code blocks. This is what gets rendered into the public-facing book — both the website and the PDF.
...
├── where-this-lands.qmd                ← Closing reflection + navigation gate to research/
